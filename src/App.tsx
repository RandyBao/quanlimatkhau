...
export default function App() {
  ...
  const handleLock = () => {
    ...
    triggerToast('Đã khóa kho dữ liệu an toàn.', 'info');
  };
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-18 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 bg-emerald-500/10 rounded-xl border border-emerald-500/20 flex items-center justify-center text-emerald-400">
              <Shield className="h-5 w-5 fill-emerald-500/10" />
            </div>
            <div className="relative">
              <div className="flex items-center gap-1.5 group">
                <h1 className="text-base font-bold text-white tracking-tight sm:text-lg uppercase">
                  {vaultTitle}
                </h1>
                <button
                  id="rename-app-btn"
                  ...
                  className="p-1 hover:bg-slate-900 rounded-lg text-slate-500 hover:text-emerald-400 transition-colors cursor-pointer"
                  title="Gợi ý & Thay đổi tên ứng dụng"
                >
                  <Edit2 className="h-3 w-3" />
                </button>
              </div>
              <p className="text-[10px] text-slate-500 font-medium uppercase tracking-wider">Mã hóa AES-256 nội bộ</p>

              {showTitleSuggestions && (
                <div className="absolute top-12 left-0 w-72 bg-slate-950 border border-slate-800 p-4 rounded-xl shadow-2xl z-50 text-left animate-slide-up">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="hidden md:inline">Tạo mật khẩu</span>
            </button>
            ...
            >
              ...
              <span className="hidden md:inline">Sao lưu lữu trữ</span>
            </button>

            {/* Safely Lock App Button */}
            <button
              id="app-lock-btn"
              type="button"
              onClick={handleLock}
              className="p-2 bg-slate-900 hover:bg-rose-500/10 border border-slate-800 hover:border-rose-500/25 text-slate-400 hover:text-rose-400 rounded-xl transition-all cursor-pointer flex items-center gap-1.5 text-xs font-semibold"
              title="Khóa lại"
            >
              <LogOut className="h-4 w-4" />
              <span className="hidden md:inline">Khóa kho</span>
            </button>
          </div>
        </div>
      </header>
