...
import { 
  ...
  Eye, EyeOff, Copy, Check, Edit2, Trash2, ExternalLink, Wallet,
  Fingerprint, Table, Maximize2, ChevronDown, ChevronUp
} from 'lucide-react';
...
export default function VaultItemCard({ 
  ...
  const getCompactSummary = () => {
    ...
    if (entry.category === 'note') {
      return entry.content ? (entry.content.length > 60 ? entry.content.slice(0, 60) + '...' : entry.content) : '';
    }
    return '';
  };
          ...
          <div className="space-y-1 mt-3 pt-3 border-t border-slate-850/60 animate-fade-in select-text">
            ...
            {entry.category === 'note' && (
              <div className="py-1 text-left">
                ...
                <div className="bg-slate-950/60 border border-slate-800/60 rounded-xl p-3 max-h-36 overflow-y-auto text-base text-slate-300 font-sans whitespace-pre-wrap leading-relaxed select-text scrollbar-thin">
                  {entry.content}
                </div>
                <div className="flex justify-end mt-1.5">
                  <button
                    ...
                    className="p-1 text-slate-500 hover:text-emerald-400 transition-colors flex items-center gap-1 text-xs font-medium"
                  >
